...
Use this file when the review feature is `RBD`.

Primary QV/JSON targets:
- `road_edges[]`
- road-boundary role/type, polynomial coefficients, range, confidence, existence, and drawing points

The original rule family is shared with LD as `LD_RBD_*`, but the VLM review context should inspect road-edge evidence first when the focus feature is `RBD`.
...
Review focus:
- for RBD, whether visible road edges, curbs, barriers, medians, or drivable-road boundaries are absent from `road_edges[]`.
- compare raw boundary structures, QV overlay geometry, and JSON road-edge existence/range.
...
Review focus:
- whether road-edge overlay geometry appears on non-boundary pixels or irrelevant structures.
- whether JSON road-edge elements have no matching visible boundary evidence.
...
Key values:
- `Start`, `End`, `Range`, road-edge drawing point extent, confidence/existence.
...
Review focus:
- whether road-edge/curb/barrier type output matches the visible boundary.
...
Review focus:
- whether road-edge role assignment matches left/right boundary and drivable-road geometry.
- for RBD, do not accept host/adjacent lane-only evidence as road-boundary evidence.
...
Review focus:
- whether road-edge existence, type, role, coefficient, or confidence fields stay invalid/stale.
